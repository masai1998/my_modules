...
# This module is in order to generate the probabilistic map from second level analysis results (zstat file)
...
def probabilistic_map_pipeline(task_list_path, results_list_path, z_threshold_value, participants_number):
    ...
    task_list = task_list.values.tolist()
    results_list = pd.read_csv(results_list_path, header = None)
    results_list = results_list.values.tolist()
    ...
    for task in task_list:
        print('Processing task is: ' + task[0])
        task_in_results_list = find_task_in_results_path(task[0], results_list)
        ...
        print('Saving as ' + task[0] + '.dscalar.nii')
        save_to_dscalar(task[0], task_data, task_header)
    print('Finish!')
...
def find_task_in_results_path(task, results_list):
    ...
    for path in results_list:
        if task in path[0]:
            task_in_results_list.append(path[0])
