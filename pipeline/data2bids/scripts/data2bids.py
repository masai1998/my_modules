...
def data2bids(args):
    ...
    # prepare logging
    log_config(pjoin(args.nifti, 'data2bids.log'))
    ...
    if args.subject or args.session:
        ...
        if args.quality_filter != 'all':
            ...
            logging.info('DATA2BIDS: filtered scaninfo is in {}'.format(pjoin(args.nifti, 'scaninfo_filtered.xlsx')))
            scaninfo = scaninfo[scaninfo['quality'] == args.quality_filter]
            scaninfo.reset_index(drop=True)
            scaninfo.to_excel(pjoin(args.nifti, 'scaninfo_filtered.xlsx'))
    else:
        if args.quality_filter != 'all':
            logging.info('DATA2BIDS: quality filter is {}'.format(args.quality_filter))
            logging.info('DATA2BIDS: filtered scaninfo is stored in {}'.format(pjoin(args.nifti, 'scaninfo_filtered.xlsx')))
    ...
    logging.info('DATA2BIDS: session-input mapping is stored in {}'.format(pjoin(args.nifti, 'session-input.json')))
    dict2json(session_input, pjoin(args.nifti, 'session-input.json'))
    ...
    for key, value in heu_session_task.items():
        print(bcolors.BOLD + "    {:s} ".format(key) + bcolors.ENDC + "contains: {}".format(value))
    logging.info('DATA2BIDS: session-task mapping is stored in {}'.format(pjoin(args.nifti, 'session-task.json')))
    dict2json(session_task, pjoin(args.nifti, 'session-task.json'))
    ...
    logging.info('DATA2BIDS: task-feature mapping is stored in {}'.format(pjoin(args.nifti, 'task-feature.json')))
    dict2json(task_feature, pjoin(args.nifti, 'task-feature.json'))
    ...
    for _value in tqdm([__ for __ in session_input.values()]):
        # upack
        if not glob.glob(pjoin(args.dicom, _value.replace('.tar.gz', ''))):
            cmd = "tar -xzvf {:s} -C {:s}".format(pjoin(args.orig, _value), args.dicom)
    ...
    for key, value in heu_session_task.items():
        check_path(pjoin(args.nifti, 'code', key))
        file = pjoin(args.nifti, 'code', key, 'heuristic.py')
    ...
    for _key, _value in tqdm(session_input.items()):
        dicom_files = pjoin(args.dicom, _value).replace('.tar.gz', '/*.IMA')
        ...
        if not args.skip_feature_validation:
            ...
            if args.overwrite:
                cmd = "heudiconv --files {:s} -o {:s} -f convertall -s {:s} -ss {:s} -c none --overwrite" \
                    .format(dicom_files, args.nifti, subID, sesID)
            else:
            ...
            if not args.preview:
                ...
                dicominfo = pd.read_csv("{:s}/.heudiconv/{:s}/info/dicominfo_ses-{:s}.tsv" \
                                        .format(args.nifti, subID, sesID), sep='\t')
                    ...
                    if 'anat' in _task:
                        if not any([_feature[0] == __[0] for __ in dicominfo_scan_feature]):
                            ...
                            logging.critical("'{:s}' protocol name mismtach! Found no {:s} in {:s}/.heudiconv/{:s}/info/dicominfo_ses-{:s}.tsv" \
                                             .format(_task, _feature[0], args.nifti, subID, sesID))
                            print(bcolors.FAIL + \
                                  "[ERROR] '{:s}' protocol name mismtach! Found no {:s} in {:s}/.heudiconv/{:s}/info/dicominfo_ses-{:s}.tsv" \
                                  .format(_task, _feature[0], args.nifti, subID, sesID) + bcolors.ENDC)
                    else:
                        if not _feature in dicominfo_scan_feature:
                            _check.append(_feature in dicominfo_scan_feature)
                            logging.critical("'{:s}' protocol name mismtach! Found no {:s} in {:s}/.heudiconv/{:s}/info/dicominfo_ses-{:s}.tsv" \
                                             .format(_task, _feature, args.nifti, subID, sesID))
                            print(bcolors.FAIL + \
                                  "[ERROR] '{:s}' protocol name mismtach! Found no {:s} in {:s}/.heudiconv/{:s}/info/dicominfo_ses-{:s}.tsv" \
                                  .format(_task, _feature, args.nifti, subID, sesID) + bcolors.ENDC)
                if not all(_check):
        ...
        heuristicpy = pjoin(args.nifti, 'code', sesID.strip(string.digits), 'heuristic.py')
        if args.overwrite:
            cmd = "heudiconv --files {:s} -o {:s} -f {:s} -s {:s} -ss {:s} -c dcm2niix -b --overwrite" \
                .format(dicom_files, args.nifti, heuristicpy, subID, sesID)
        else:
    ...
    print('Log is saved in {}.'.format(pjoin(args.nifti, 'data2bids.log')))
